...
1. **자산 관리 대장 (`assets.xlsx`)**
   - 사내에서 관리 중인 서버 및 소프트웨어 목록
   - 필수 컬럼: `구분(OS/미들웨어/DB)`, `종류(제품명)`, `버전`
   - *예시: Nginx (1.14.0), CentOS (7.8), MongoDB (4.4.10)*
2. **보안 인텔리전스 소스 (Real-time)**
   - **EoS 정보:** `endoflife.date` 공식 API
   - **취약점 뉴스:** KISA(인터넷보호나라) 보안 권고문 RSS

### 📤 Output (출력)
1. **EoS 현황 리포트 (Static Analysis)**
   - 기술지원 종료(Expired) 및 임박(Warning) 자산 식별
   - *예시: "🚨 [만료됨] CentOS 7.8 (종료일: 2024-06-30)"*
2. **맞춤형 취약점 경고 (Dynamic Analysis via AI)**
...
⏳ [Phase 1] 소프트웨어 기술지원 종료(EoS) 점검 중...
   ...
   ✅ [양호] Rocky Linux 8.1
   🚨 [만료됨] MariaDB 10.4.19 (종료일: 2024-06-18)
   ✅ [양호] Windows Server 2016.0
   🚨 [만료됨] MongoDB 3.2.20 (종료일: 2018-09-30)
   🚨 [만료됨] Ubuntu 20.04 (종료일: 2025-05-31)
   ⚠️ [임박함] Redis 7.2.4 (남은기간: 54일)
   ✅ [양호] React 18.2.0
   🚨 [만료됨] MongoDB 4.4.10 (종료일: 2024-02-29)
   -> 총 7건의 EoS 위험/경고 자산이 발견되었습니다.
